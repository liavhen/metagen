""" Adopted for https://github.com/kch3782/torcwa/blob/main/example/Materials.py """
...
class Material(torch.autograd.Function):
    ...
    def forward(ctx, material, wavelength, dl = 0.005):
        materials_dir = join(PROJECT_DIR, 'data', 'materials')
        # assert material in os.listdir(materials_dir), f"Material {material} not found in the materials directory"

        # open material data
        f = open(join(materials_dir, material + '.txt'), 'r')
        data = f.readlines()
        f.close()
        nk_data = []
        for i in range(len(data)):
            try:
                ...
                ValueError(f"Error in line {i} of {material}.txt: {data[i]} cannot be parsed!")
        ...
        return torch.tensor(nk_value,dtype=torch.complex64)
